...
var GridBuilder = (function () {
    ...
    function onPathUpdate() {
        ...
        for (var idxTop in alignedTopSeq) {
            ...
            if (c1 === mGapSymbol || c2 === mGapSymbol) {
                if (idxTop > 0 && c1 === mGapSymbol && alignedTopSeq[idxTop-1] === mGapSymbol){
                    score += mGapExtScore;
                } else if (idxTop > 0 && c2 === mGapSymbol && alignedSideSeq[idxTop-1] === mGapSymbol){
                    score += mGapExtScore;
                } else{
                    score += mGapOpenScore + mGapExtScore;
                }
            } else if (c1 === c2) {
    ...
    mSelf = {
        ...
        startCustomPath: function() {
            this.rebuildTable(mDomContainer, mDomResultContainer, mMatchScore, mMismatchScore, mGapOpenScore, mGapExtScore, mSideSequence, mTopSequence);
            mIsCustomPathMode = true;
        },

        rebuildTable: function(domContainer, resultContainer, matchScore, mismatchScore, gapOpenScore, gapExtScore, seqSide, seqTop) {
            ...
            mGapOpenScore = gapOpenScore;
            mGapExtScore = gapExtScore;
            ...
            for (var i = 0; i < width; i++) {
                ...
                for (var j = 0; j < height; j++) {
                    ...
                    if (i === 0) {
                        mGap1[i][j] = mDiag[i][j-1] + (mGapOpenScore + mGapExtScore);
                        mGap2[i][j] = -Infinity;
                        if (j > 1){
                            mGap1[i][j] = Math.max(mGap1[i][j], mGap1[i][j-1] + mGapExtScore);
                        }
                        mDiag[i][j] = mGap1[i][j];
                        mPathTable[i][j] = Math.max(mGap1[i][j], mGap2[i][j], mDiag[i][j]);
                        mCellMap[i + "_" + j] = {
                            'winningScore': mPathTable[i][j]
                        };
                        continue;
                    ...
                    if (j === 0) {
                        mGap1[i][j] = -Infinity;
                        mGap2[i][j] = mDiag[i-1][j] + (mGapOpenScore + mGapExtScore);
                        if (i > 1){
                            mGap2[i][j] = Math.max(mGap2[i][j], mGap2[i-1][j] + mGapExtScore);
                        }
                        mDiag[i][j] = mGap2[i][j];
                        mPathTable[i][j] = Math.max(mGap1[i][j], mGap2[i][j], mDiag[i][j]);
                        mCellMap[i + "_" + j] = {
                    ...
                    */
                    mGap1[i][j] = Math.max(mGap1[i][j - 1] + mGapExtScore, mDiag[i][j - 1] + (mGapOpenScore + mGapExtScore));
                    mGap2[i][j] = Math.max(mGap2[i - 1][j] + mGapExtScore, mDiag[i - 1][j] + (mGapOpenScore + mGapExtScore));
                    mDiag[i][j] = Math.max(mGap1[i][j], mGap2[i][j], parseInt(comparisonScore, 10) + parseInt(mDiag[i - 1][j - 1]));
                    var moveUpScore = mGap1[i][j];
                    var moveSdScore = mGap2[i][j];
                    var moveDgScore = mDiag[i][j];
                    mPathTable[i][j] = Math.max(moveUpScore, moveSdScore, moveDgScore);

                    /*console.log(
                        "Processing cell(" + i + ", " + j + ")\n"
                        + "Side score is " + moveSdScore + "\n"
                        + "Up score is " + moveUpScore + "\n"
                        + "Diag score is " + moveDgScore + "\n"
                    );*/
                    ...
                    */
                    ...
                    var gapSd = moveSdScore - mPathTable[i-1][j];
                    var gapUp = moveUpScore - mPathTable[i][j-1];

                    mCellMap[i + "_" + j] = {
                        'sideScoreText': mPathTable[i - 1][j] + " + " + gapSd + (gapSd === mGapExtScore ? '(Gap Extension)' : '(Gap Open)') + " = " + moveSdScore,
                        'upScoreText': mPathTable[i][j - 1] + " + " + gapUp + (gapUp === mGapExtScore ? '(Gap Extension)' : '(Gap Open)') + " = " + moveUpScore,
